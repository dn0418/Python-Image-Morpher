...
## Removed
- Removed an unrelated folder from the repository (oops!)
## Fixes
...
reduction in SLOC). 
    ...
    - Removed unnecessary "changes" to the left/right images after painting
        - Enabling the image and the scaling its contents is handled by the image loaders and doesn't need to be here
...
- Interpolated data is now saved directly to the image variables in Morphing.py's getImageAtAlpha().
    - The function was assigning all final interpolated image values to a "copy", which was 
then saved to the corresponding images variables. This "copy" was redundant and unnecessary, so it has been removed.
